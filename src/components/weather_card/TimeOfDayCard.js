...
class TimeOfDayCard extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={`card d-flex flex-md-column bd-highlight align-items-center justify-content-center`}
      >
        <div className="card-component mx-auto p-2 text-center font-weight-bold">
          {timeOfDay}
        </div>
          ...
          <span>
            {roundNumbers(
              this.context.weatherUnit === "C"
                ? this.props.temp
                : convertToFahrenheit(this.props.temp)
            )}
          </span>
        </div>
      </div>
