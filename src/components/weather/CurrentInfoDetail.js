      ...
      <div className="detail-container d-flex flex-row p-2 justify-content-center align-items-center">
        <div className="d-flex flex-column align-items-center w-50">
          <img
            ...
            className="img-fluid w-75 h-75 p-2"
          />
          <p className="my-2 text-center text-capitalize larger-font">
        ...
        <div className="d-flex flex-column align-items-center">
          <p className="my-2">
            <span className="h3">
              {Math.round(
                weatherUnit === "C" ? avgTemp : convertToFahrenheit(avgTemp)
              )}
            </span>
            <span className="font-weight-light">
              <span
                className={`cursor-pointer ${
                  weatherUnit === "C"
                    ? "font-weight-normal border-light"
                    : "selected"
                }`}
                onClick={() => unitClick("C")}
              >
                <WeatherIcon iconName="celsius" />
              </span>{" "}
              &nbsp;|&nbsp;
              <span
                className={`cursor-pointer ${
                ...
                onClick={() => unitClick("F")}
              >
                <WeatherIcon iconName="fahrenheit" />
              </span>
            </span>
          </p>
          <p className="my-2">
            <span className="font-weight-normal">
              {currentDate.format("MMM Do") || ""}
            </span>
          </p>
        </div>
      </div>
      <div className="detail-container d-flex flex-column justify-content-center align-items-center p-2">
        <div className="ml-sm-3 ml-md-3 ml-lg-3">
          {currentWeather.pressure ? (
            <div className="my-1">
              <p className="mb-2">
                <span className="font-weight-bold">Air pressure:</span>{" "}
          ...
          {currentWeather.wind_speed ? (
            <div className="my-1">
              <p className="mb-2">
                ...
                {weatherUnit === "C"
                  ? convertToMetric(currentWeather.wind_speed).toFixed(2) +
                    " mps"
                  : currentWeather.wind_speed.toFixed(2) + " mph"}
                &nbsp;
                <WeatherDirectionIcon iconName={currentWeather.wind_deg} />{" "}
              </p>
            </div>
          ) : null}
        </div>
